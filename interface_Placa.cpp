...
// construtor da classe interface_placa
interface_Placa::interface_Placa(){
	input = 0;
	iCount = 0;
	output = 0;
	disp = "nada";
...
void interface_Placa::setCoin(int coin){
	if (coin == 1){
...
void interface_Placa::selectOutput(int output){
	float x, y;
...
string interface_Placa::readKeyboard(){
	//cout << "readkey"<<endl;
...
string interface_Placa::displayAds(){
	string ads;
	int x;
...
ostream& operator<< (ostream& c, interface_Placa tempo){
	c << tempo.hr << ":";
	c << tempo.min << ":";
	c << tempo.sec << "  ";
	c << tempo.day << "/";
...
void interface_Placa::displayTimeAndDate(){
	theTime = time(NULL);
...
string interface_Placa::getNameFile(){
	string dia, mes;
...
void interface_Placa::saveFile(int refri1, int refri2){
